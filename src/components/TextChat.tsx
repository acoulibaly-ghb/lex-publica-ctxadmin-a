...
const SimpleMarkdown = ({ text, isUser }: { text: string, isUser: boolean }) => {
  ...
  // Increased line-height (leading-8) for model messages to improve readability of dense legal text
    ...
    if (showScore) {
        return (
            <div className="bg-gradient-to-br from-indigo-50 to-white p-8 rounded-3xl border border-indigo-100 text-center shadow-sm">
                ...
                <h3 className="text-2xl font-bold text-indigo-900 mb-2">Quiz TerminÃ© !</h3>
                ...
                <p className="text-sm font-medium text-slate-500 bg-slate-50 py-2 px-4 rounded-lg inline-block">
                    {score === data.length ? "ðŸŽ‰ Excellent ! MaÃ®trise parfaite." : score > data.length/2 ? "ðŸ‘ Bon travail !" : "ðŸ“š Continuez Ã  rÃ©viser le glossaire."}
                </p>
            </div>
        );
    }
...
const TextChat: React.FC = () => {
  ...
  const deleteSession = (e: React.MouseEvent, id: string) => {
      ...
      if (currentSessionId === id) { if (newSessions.length > 0) loadSession(newSessions[0]); else createNewSession(); }
  };
  ...
  const handlePlayMessage = async (text: string, msgIdx: number) => {
    if (playingMessageId === msgIdx) return; 
  ...
  const handleQuizMode = () => {
    sendMessage("GÃ©nÃ¨re 3 questions de QCM sur le droit du contentieux international au format JSON strict.");
  };
  ...
  const handleWhoAmI = () => sendMessage("Qui Ãªtes-vous ?");

  const sendMessage = async (text: string) => {
    if ((!text.trim() && !attachment) || isLoading) return;
    if (!API_KEY) {
        const errorMsg: Message = { role: 'model', text: "### Erreur de Configuration\n\nLa clÃ© API est manquante.", timestamp: new Date() };
    ...
    try {
      ...
      if (responseText.trim().startsWith('[') && responseText.trim().endsWith(']')) {
          try {
              ...
              botMsg.text = ""; // Empty text, only Quiz Display
          } catch (e) {
      ...
      console.error(error);
      updateCurrentSession([...newHistory, { role: 'model', text: "Une erreur est survenue.", timestamp: new Date() }]);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="flex h-[600px] relative bg-slate-50/50 overflow-hidden rounded-b-3xl">
      ...
      <div className={`absolute inset-y-0 left-0 z-30 w-80 bg-white shadow-2xl transform transition-transform duration-300 ease-in-out ${showHistory ? 'translate-x-0' : '-translate-x-full'} flex flex-col border-r border-slate-100`}>
          ...
          <div className="flex-1 overflow-y-auto px-3 space-y-2 pb-4">
              {sessions.map(session => (
                  <div key={session.id} className={`group flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all border border-transparent ${currentSessionId === session.id ? 'bg-indigo-50 text-indigo-800 border-indigo-100 shadow-sm' : 'hover:bg-slate-50 text-slate-600 hover:border-slate-100'}`} onClick={() => loadSession(session)}>
                      {/* ... Session Item content ... */}
                      {editingSessionId === session.id ? (
                          <input type="text" value={editTitleInput} onChange={(e) => setEditTitleInput(e.target.value)} onBlur={() => saveTitle(session.id)} onKeyDown={(e) => handleEditKeyDown(e, session.id)} autoFocus className="flex-1 min-w-0 text-sm px-1 py-0.5 border border-indigo-300 rounded" onClick={(e) => e.stopPropagation()} />
                      ) : (
                      ...
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={(e) => handleExport(e, session)} className="p-1.5 text-slate-400 hover:text-emerald-500 rounded-lg" title="PDF"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg></button>
                        <button onClick={(e) => startEditing(e, session)} className="p-1.5 text-slate-400 hover:text-indigo-500 rounded-lg" title="Renommer"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg></button>
                        <button onClick={(e) => deleteSession(e, session.id)} className="p-1.5 text-slate-400 hover:text-red-500 rounded-lg" title="Supprimer"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
                      </div>
                  </div>
              ))}
          </div>
      ...
      <div className="flex-1 flex flex-col relative w-full min-w-0 bg-white">
         <div className="absolute top-4 left-4 z-10">
            <button onClick={() => setShowHistory(!showHistory)} className="p-2 bg-white/80 backdrop-blur shadow-sm border border-slate-200 rounded-lg text-slate-500 hover:text-indigo-600 transition-all">
         ...
         <div className="p-4 bg-white border-t border-slate-100 relative z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
            ...
            {messages.length < 3 && !isLoading && !attachment && <div className="flex gap-2 mb-4 overflow-x-auto pb-2 scrollbar-hide items-center">
                <button onClick={handleQuizStart} className="whitespace-nowrap px-3 py-1.5 bg-rose-100 text-rose-700 text-xs font-bold rounded-full border border-rose-200 hover:bg-rose-200 hover:border-rose-300 transition-colors flex items-center gap-1 shadow-sm"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"></path></svg>Collez-moi !</button>
                <button onClick={handleWhoAmI} className="whitespace-nowrap px-3 py-1.5 bg-teal-100 text-teal-700 text-xs font-bold rounded-full border border-teal-200 hover:bg-teal-200 hover:border-teal-300 transition-colors flex items-center gap-1 shadow-sm"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>Qui suis-je ?</button>
                <div className="w-px h-6 bg-slate-200 mx-1"></div>
                {suggestions.map((s, i) => <button key={i} onClick={() => sendMessage(s)} className="whitespace-nowrap px-3 py-1.5 bg-indigo-50 text-indigo-600 text-xs font-medium rounded-full border border-indigo-100 hover:bg-indigo-100 hover:border-indigo-200 transition-colors">{s}</button>)}
            </div>}
