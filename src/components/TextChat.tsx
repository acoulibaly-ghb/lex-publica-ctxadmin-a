...
const SimpleMarkdown = ({ text, isUser }: { text: string, isUser: boolean }) => {
  ...
  const flushList = (key: string) => {
    if (currentList.length > 0) {
      elements.push(<ul key={key} className="list-disc pl-6 mb-4 space-y-2 marker:text-indigo-400 text-slate-700">{[...currentList]}</ul>);
      currentList = [];
    }
  };
  lines.forEach((line, index) => {
    ...
    if (line.startsWith('### ')) {
      ...
      elements.push(<h3 key={`h3-${index}`} className={`text-lg font-bold mt-6 mb-3 ${isUser ? 'text-white' : 'text-indigo-800 border-b border-indigo-100 pb-1'}`}>{parseBold(line.replace('### ', ''))}</h3>);
    } else if (line.startsWith('## ')) {
        flushList(`list-before-${index}`);
        elements.push(<h2 key={`h2-${index}`} className={`text-xl font-extrabold mt-8 mb-4 ${isUser ? 'text-white' : 'text-indigo-900'}`}>{parseBold(line.replace('## ', ''))}</h2>);
    } else if (line.startsWith('#### ') || line.startsWith('##### ')) {
        flushList(`list-before-${index}`);
        elements.push(<p key={`h-bold-${index}`} className={`font-bold mt-4 mb-2 text-base ${isUser ? 'text-white' : 'text-indigo-700'}`}>{parseBold(line.replace(/#{4,5} /, ''))}</p>);
    } else if (line.startsWith('- ') || line.startsWith('* ')) {
      currentList.push(<li key={`item-${index}`} className="pl-1 leading-relaxed">{parseBold(line.replace(/^[-*] /, ''))}</li>);
    } else {
      if (trimmedLine !== '') {
        flushList(`list-before-${index}`);
        elements.push(<p key={`p-${index}`} className="mb-4 last:mb-0 leading-relaxed text-slate-700">{parseBold(line)}</p>);
...
const GlossaryModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  const filtered = GLOSSARY_DATA.filter(item => item.term.toLowerCase().includes(search.toLowerCase()) || item.definition.toLowerCase().includes(search.toLowerCase()));
  return (
    <div className="absolute inset-0 z-50 bg-slate-900/20 backdrop-blur-sm flex justify-end transition-all duration-300">
      <div className="w-full md:w-[400px] bg-white h-full shadow-2xl p-6 flex flex-col animate-in slide-in-from-right duration-300 border-l border-slate-100">
        <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-100">
          <h2 className="text-xl font-bold text-indigo-900 flex items-center gap-2">Glossaire</h2>
          <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><svg className="w-6 h-6 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
        </div>
        <div className="relative mb-6"><input type="text" placeholder="Rechercher..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-4 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 outline-none text-sm" /></div>
...
const QuizDisplay = ({ data }: { data: QuizQuestion[] }) => {
    ...
    const handleOptionClick = (idx: number) => {
        ...
        setSelectedOption(idx); setIsAnswered(true);
        if (idx === question.correctAnswerIndex) setScore(s => s + 1);
    };
    const nextQuestion = () => {
        if (currentIndex < data.length - 1) { setCurrentIndex(prev => prev + 1); setSelectedOption(null); setIsAnswered(false); } else { setShowScore(true); }
    };
    if (showScore) {
        return (
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm mt-4 max-w-xl mx-auto">
            ...
            <div className="space-y-3">
                {question.options.map((opt, idx) => {
                    ...
                    } else { btnClass += "bg-white border-slate-100 hover:border-indigo-400 hover:bg-indigo-50 text-slate-700 hover:shadow-md"; }
                    return <button key={idx} onClick={() => handleOptionClick(idx)} className={btnClass} disabled={isAnswered}>{opt}</button>;
                })}
            </div>
            {isAnswered && <div className="mt-6"><div className="p-5 rounded-xl text-sm bg-slate-50 border border-slate-200 mb-4">{question.explanation}</div><button onClick={nextQuestion} className="w-full py-3.5 bg-indigo-600 text-white rounded-xl font-bold">Suivant</button></div>}
        </div>
    );
};
...
const TextChat: React.FC = () => {
  ...
  const suggestions = ["Qu'est-ce qu'un différend ?", "L'affaire Mavrommatis", "Avis consultatif vs Arrêt", "Juge ad hoc"];
  ...
  useEffect(() => {
    ...
    if (savedSessions) {
        try {
            ...
            if (hydratedSessions.length > 0) loadSession(hydratedSessions[0]); else createNewSession();
  ...
  const saveSessionsToStorage = (updatedSessions: ChatSession[]) => {
      ...
      if (allSaved) { try { const parsed = JSON.parse(allSaved); otherSessions = parsed.filter((s: any) => s.mode !== ChatMode.TEXT); } catch(e) {} }
  ...
  const handlePlayMessage = async (text: string, msgIdx: number) => {
    if (playingMessageId === msgIdx) return; setPlayingMessageId(msgIdx);
    const cleanText = cleanTextForTTS(text);
    try {
        if (!API_KEY) return;
        const ai = new GoogleGenAI({ apiKey: API_KEY });
        const response = await ai.models.generateContent({ model: "gemini-2.5-flash-preview-tts", contents: [{ parts: [{ text: cleanText }] }], config: { responseModalities: ["AUDIO"], speechConfig: { voiceConfig: { prebuiltVoiceConfig: { voiceName: 'Kore' } } } } });
        ...
        source.start();
    } catch (e) { console.error(e); setPlayingMessageId(null); }
  };

  const handleQuizStart = () => sendMessage("Lance une session de quiz interactif au format JSON strict (3 questions).");
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      const result = await ai.models.generateContentStream({ model: 'gemini-2.5-flash', contents: [{ role: 'user', parts: parts }], config: { systemInstruction: SYSTEM_INSTRUCTION } });
      let fullText = '';
      ...
      for await (const chunk of result) {
          ...
          if (chunkText) { fullText += chunkText; updateCurrentSession(msgsWithModel.map((m, i) => i === msgsWithModel.length - 1 ? { ...m, text: fullText } : m)); }
      }
      ...
      setIsLoading(false);
    } catch (error) { console.error(error); updateCurrentSession([...newHistory, { role: 'model', text: "Erreur.", timestamp: new Date() }]); setIsLoading(false); }
  };
  ...
  return (
    <div className="flex h-[600px] relative bg-slate-50/50 overflow-hidden rounded-b-3xl">
      ...
      <div className={`absolute inset-y-0 left-0 z-30 w-80 bg-white shadow-2xl transform transition-transform duration-300 ease-in-out ${showHistory ? 'translate-x-0' : '-translate-x-full'} flex flex-col border-r border-slate-100`}>
          <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50"><h2 className="font-bold text-slate-700">Historique</h2><button onClick={() => setShowHistory(false)} className="p-1 text-slate-400 hover:bg-slate-100 rounded-lg"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"/></svg></button></div>
          <div className="p-4"><button onClick={createNewSession} className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all text-sm font-semibold shadow-sm">Nouvelle discussion</button></div>
      ...
      <div className="flex-1 flex flex-col relative w-full min-w-0 bg-white">
        <div className="absolute top-4 left-4 z-10"><button onClick={() => setShowHistory(!showHistory)} className="p-2 bg-white/80 backdrop-blur shadow-sm border border-slate-200 rounded-lg text-slate-500 hover:text-indigo-600 transition-all"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16"/></svg></button></div>
        <div className="absolute top-4 right-4 z-10"><button onClick={() => setShowGlossary(true)} className="flex items-center gap-2 px-3 py-2 bg-white/90 backdrop-blur shadow-sm border border-indigo-100 rounded-lg text-indigo-600 hover:bg-indigo-50 transition-all text-sm font-semibold"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>Glossaire</button></div>
        <div className="flex-1 overflow-y-auto p-6 space-y-8 scroll-smooth pt-16">
            ...
            <div key={idx} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'} animate-in fade-in slide-in-from-bottom-4 duration-500`}>
                <div className="flex-shrink-0 mt-1">{msg.role === 'user' ? <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-white shadow-md"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg></div> : <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-emerald-500 to-teal-500 flex items-center justify-center text-white shadow-md"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3"/></svg></div>}</div>
                <div className={`max-w-[85%] rounded-3xl px-6 py-5 shadow-sm ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-lg' : 'bg-white text-slate-800 rounded-tl-lg border border-slate-200'} relative group ${msg.role === 'user' ? '' : 'w-full'}`}>
                    {msg.isQuiz && msg.quizData ? <QuizDisplay data={msg.quizData} /> : (
                        <>
                            ...
                            <div className="flex items-center justify-between mt-2">
                                ...
                                {msg.role === 'model' && !isLoading && (
                                    <button onClick={() => handlePlayMessage(msg.text, idx)} className={`p-1.5 rounded-full transition-all ml-2 ${playingMessageId === idx ? 'bg-indigo-100 text-indigo-600 animate-pulse ring-2 ring-indigo-200' : 'hover:bg-slate-100 text-slate-400 hover:text-indigo-600'}`} title="Lire à haute voix">
