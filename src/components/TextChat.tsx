        ...
        return (
            <div className="bg-gradient-to-br from-indigo-50 to-white p-8 rounded-3xl border border-indigo-100 text-center shadow-sm">
                ...
                <p className="text-sm font-medium text-slate-500 bg-slate-50 py-2 px-4 rounded-lg inline-block">
                    {score === data.length ? "🎉 Excellent ! Maîtrise parfaite." : score > data.length/2 ? "👍 Bon travail !" : "📚 Continuez à réviser le glossaire."}
                </p>
            </div>
        );
    }
...
const TextChat: React.FC = () => {
  ...
  const [isPlayingAudio, setIsPlayingAudio] = useState(false); // ✅ AJOUTÉ - Bug fix #2
  ...
  const handlePlayMessage = async (text: string, msgIdx: number) => {
    ...
    setIsPlayingAudio(true); // ✅ AJOUTÉ - Active l'indicateur audio
    ...
    try {
        ...
        source.onended = () => {
            ...
            setIsPlayingAudio(false); // ✅ AJOUTÉ - Désactive l'indicateur
        };
        source.start();
    } catch (e) {
        console.error(e);
        setPlayingMessageId(null);
        setIsPlayingAudio(false); // ✅ AJOUTÉ - Désactive en cas d'erreur
    }
  };
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      let fullText = '';
      let sentenceBuffer = ''; // Unused for auto-TTS now, but kept for stream logic structure
      ...
      // Post-processing to detect JSON Quiz ONLY after full generation
      if (fullText.trim().startsWith('[') && fullText.trim().endsWith(']')) {
          try {
              ...
              // Replace the streaming text message with the quiz component
...
<div className="p-4">
          <button 
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all text-sm font-semibold shadow-sm active:scale-95 transform transition-transform"
          >
  ...
  <div className="flex-1 flex flex-col relative w-full min-w-0 bg-white">
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-8 scroll-smooth pt-16">
        ...
        >
            ...
            } relative group ${msg.role === 'user' ? '' : 'w-full'}`}
            >
                {msg.isQuiz && msg.quizData ? (
                    ...
                    <>
                        ...
                        <div className="flex items-center justify-between mt-2">
                            <div className={`text-[10px] opacity-70 ${msg.role === 'user' ? 'text-indigo-100 text-right' : 'text-slate-400'}`}>
    ...
    <div className="p-4 bg-white border-t border-slate-100 relative z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        ...
        {!isLoading && !attachment && (
            <div className="flex gap-2 mb-4 overflow-x-auto pb-2 scrollbar-hide items-center">
                ...
                {/* Gardé pour compatibilité mais peut être commenté */}
                {/* 
                <button 
